...
from airflow_watcher.utils.rbac import get_accessible_dag_ids, filter_results_rbac


watcher_api_blueprint = Blueprint("watcher_api", __name__, url_prefix="/api/watcher")


def _safe_int(value, default: int, min_val: int = 1, max_val: int = 10000) -> int:
    """Safely parse an int from a query param with bounds checking."""
    clamped_default = max(min_val, min(default, max_val))
    if value is None:
...
def get_health():
    ...
    try:
        ...
        return jsonify({"status": "success", "data": payload, "timestamp": timezone.utcnow().isoformat()}), http_status

    except Exception as e:
        import logging as _logging
        _logging.getLogger(__name__).error("Health endpoint error: %s", e, exc_info=True)
        return jsonify({
            "status": "error",
            "message": "Internal error while computing health",
            "timestamp": timezone.utcnow().isoformat(),
        }), 503
...
def get_long_running_tasks():
    ...
    tasks = filter_results_rbac(tasks, allowed_dags)
...
def get_retry_heavy_tasks():
    ...
    tasks = filter_results_rbac(tasks, get_accessible_dag_ids())
...
def get_zombie_tasks():
    ...
    zombies = filter_results_rbac(zombies, get_accessible_dag_ids())
...
def get_inactive_dags():
    ...
    inactive = filter_results_rbac(inactive, get_accessible_dag_ids())
...
def get_upstream_failures():
    ...
    failures = filter_results_rbac(failures, get_accessible_dag_ids())
