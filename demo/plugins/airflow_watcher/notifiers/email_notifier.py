...
class EmailNotifier:
    ...
    def _build_failure_html(self, failure: DAGFailure) -> str:
        ...
        failed_tasks_html = "".join(
            f"<li><code>{esc(t.task_id)}</code> (attempt {t.try_number}/{t.max_tries})</li>"
        ...
        <body style="font-family: Arial, sans-serif; padding: 20px;">
            ...
            <table style="border-collapse: collapse; width: 100%; max-width: 600px;">
                <tr>
                    ...
                    <td style="padding: 8px; border: 1px solid #ddd;"><code>{esc(failure.dag_id)}</code></td>
                </tr>
                <tr>
                    <td style="padding: 8px; border: 1px solid #ddd; font-weight: bold;">Run ID</td>
                    <td style="padding: 8px; border: 1px solid #ddd;"><code>{esc(failure.run_id)}</code></td>
                </tr>
                <tr>
                    <td style="padding: 8px; border: 1px solid #ddd; font-weight: bold;">Execution Date</td>
                    <td style="padding: 8px; border: 1px solid #ddd;">{esc(str(failure.execution_date))}</td>
                </tr>
                <tr>
                    <td style="padding: 8px; border: 1px solid #ddd; font-weight: bold;">Start Time</td>
                    <td style="padding: 8px; border: 1px solid #ddd;">{esc(str(failure.start_date))}</td>
                </tr>
                <tr>
                    <td style="padding: 8px; border: 1px solid #ddd; font-weight: bold;">End Time</td>
                    <td style="padding: 8px; border: 1px solid #ddd;">{esc(str(failure.end_date))}</td>
                </tr>
            </table>
    ...
    def _build_sla_miss_html(self, sla_miss: SLAMissEvent) -> str:
        ...
        esc = html_mod.escape
        desc_html = f"<p><strong>Description:</strong> {esc(sla_miss.description)}</p>" if sla_miss.description else ""
        return f"""
        <html>
        <body style="font-family: Arial, sans-serif; padding: 20px;">
            ...
            <table style="border-collapse: collapse; width: 100%; max-width: 600px;">
                <tr>
                    <td style="padding: 8px; border: 1px solid #ddd; font-weight: bold;">DAG ID</td>
                    <td style="padding: 8px; border: 1px solid #ddd;"><code>{esc(sla_miss.dag_id)}</code></td>
                </tr>
                <tr>
                    <td style="padding: 8px; border: 1px solid #ddd; font-weight: bold;">Task ID</td>
                    <td style="padding: 8px; border: 1px solid #ddd;"><code>{esc(sla_miss.task_id)}</code></td>
                </tr>
                <tr>
                    <td style="padding: 8px; border: 1px solid #ddd; font-weight: bold;">Execution Date</td>
                    <td style="padding: 8px; border: 1px solid #ddd;">{esc(str(sla_miss.execution_date))}</td>
                </tr>
                <tr>
                    <td style="padding: 8px; border: 1px solid #ddd; font-weight: bold;">Timestamp</td>
                    <td style="padding: 8px; border: 1px solid #ddd;">{esc(str(sla_miss.timestamp))}</td>
                </tr>
            </table>
