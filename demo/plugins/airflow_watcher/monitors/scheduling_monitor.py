...
class SchedulingMonitor:
    ...
    def get_stale_dags(
        ...
        dag_ids = [d.dag_id for d in active_dags]
        dag_map = {d.dag_id: d for d in active_dags}
        ...
        # Also get the state of the latest run per DAG
        # We need a subquery to join back for the state
        from sqlalchemy import tuple_

        latest_pairs = [(row.dag_id, row.latest_execution) for row in latest_runs]
        state_by_dag = {}
        if latest_pairs:
            latest_run_rows = (
                session.query(DagRun.dag_id, DagRun.state)
                .filter(tuple_(DagRun.dag_id, DagRun.execution_date).in_(latest_pairs))
                .all()
            )
            state_by_dag = {row.dag_id: str(row.state) for row in latest_run_rows}

        stale_dags = []
        for dag_id in dag_ids:
            dag = dag_map[dag_id]
            last_exec = latest_by_dag.get(dag_id)
            ...
            elif last_exec < cutoff_time:
                hours_since = (current_time - last_exec).total_seconds() / 3600
                stale_dags.append(
                    {
                        "dag_id": dag_id,
                        "last_run": last_exec.isoformat(),
                        "last_run_state": state_by_dag.get(dag_id, "unknown"),
