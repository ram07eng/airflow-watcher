...
.. deprecated:: Airflow 3.0
    The ``sla_miss`` table (``airflow.models.SlaMiss``) was deprecated in
    Airflow 2.6 and **removed in Airflow 3.0**.  This module gracefully
    degrades when the model is unavailable, returning empty results instead
    of crashing.  A future version of airflow-watcher will migrate to
    Airflow 3.x's Dataset-based event model.
"""
...
# Gracefully handle SlaMiss removal in Airflow 3.0+
try:
    from airflow.models import SlaMiss

    _SLA_MISS_AVAILABLE = True
except ImportError:
    _SLA_MISS_AVAILABLE = False
    SlaMiss = None  # type: ignore[assignment,misc]
    warnings.warn(
        "airflow.models.SlaMiss is not available (Airflow 3.0+). "
        "SLA monitoring features will return empty results. "
        "A future airflow-watcher release will migrate to the Dataset event model.",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    def get_sla_statistics(
        ...
        if not _SLA_MISS_AVAILABLE:
            return {
                ...
                "message": "SLA monitoring unavailable (SlaMiss model not present in this Airflow version)",
        ...
        # SLA misses by DAG
        misses_by_dag = (
            session.query(SlaMiss.dag_id, func.count(SlaMiss.dag_id).label("miss_count"))
            .filter(
                SlaMiss.timestamp >= cutoff_time,
            )
            .group_by(SlaMiss.dag_id)
            .order_by(func.count(SlaMiss.dag_id).desc())
            .limit(10)
            .all()
        )
        ...
        # SLA misses by task
        misses_by_task = (
            session.query(SlaMiss.dag_id, SlaMiss.task_id, func.count(SlaMiss.task_id).label("miss_count"))
            .filter(
                SlaMiss.timestamp >= cutoff_time,
            )
            .group_by(SlaMiss.dag_id, SlaMiss.task_id)
            .order_by(func.count(SlaMiss.task_id).desc())
            .limit(10)
            .all()
        )
        ...
        return {
            ...
            "top_dags_with_misses": [{"dag_id": dag_id, "miss_count": count} for dag_id, count in misses_by_dag],
            "top_tasks_with_misses": [
                {"dag_id": dag_id, "task_id": task_id, "miss_count": count} for dag_id, task_id, count in misses_by_task
            ],
            "timestamp": timezone.utcnow().isoformat(),
        }
